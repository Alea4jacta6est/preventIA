...
from src.back.models.llm_outputs import RisksResponse
from src.back.prompts.prompt_diabetes import SYSTEM_DIABETES, PROMPT_DIABETES
from src.back.prompts.prompt_summary import SYSTEM_SUMMARY, PROMPT_SUMMARY
from src.back.handlers import (
    load_patient_data,
    get_all_csv_files,
...
def analyze_patient_directory(patient_directory: str) -> dict:
    ...
    doctor_advices = [
        risk["doctor_advise"]
        for risk in data_response_json["risks"]
        if risk.get("doctor_advise")
    ]
    patient_advices = [
        risk["patient_advise"]
        for risk in data_response_json["risks"]
        if risk.get("patient_advise")
    ]

    logging.info("Preparing recommendations for doctors...")
    if doctor_advices:
        doc_sum_msgs = create_chat_messages(
            doctor_advices, SYSTEM_SUMMARY, PROMPT_SUMMARY
        )
        doc_sum_resp = get_chat_response(
            client, model, doc_sum_msgs, response_format="text"
        )
    ...
    if patient_advices:
        pat_sum_msgs = create_chat_messages(
            patient_advices, SYSTEM_SUMMARY, PROMPT_SUMMARY
        )
        pat_sum_resp = get_chat_response(
            client, model, pat_sum_msgs, response_format="text"
        )
...
def startup_event():
    ...
    t.start()
    logging.info(
        "Background thread started. We can now serve partial results once each file is ready."
    )
...
def get_analysis_json(patient_id: str):
    ...
    if not os.path.exists(path):
        logging.error(
            f"No JSON file found for {patient_id} at {path} (maybe still analyzing?)"
        )
        raise HTTPException(
            status_code=404, detail=f"No analysis found yet for {patient_id}"
        )
...
def root():
    return {
        "message": "Welcome to the partial results backend! Some analyses may still be in progress."
    }


if __name__ == "__main__":
    uvicorn.run("src.back.mistral_diabetes:app", host="127.0.0.1", port=8000, reload=True)
